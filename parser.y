...
%union {
	...
	struct IncludeMacro 	*Include_ptr;
	...
	struct HeaderIdent		*HeaderIdent_ptr;
...
/* TinyC Program Grammar Rules */
%%

PROGRAM: METHODDEFLIST MAINFUNC {
	   ...
	   | INCLUDELIST METHODDEFLIST MAINFUNC {
            struct Program *p = (struct Program*)malloc(sizeof(struct Program));
			p->includeMacro = $1;
            p->methodDef = $2;
            p->mainFunc = $3;
...
INCLUDESTMT: INCLUDE LT HEADERID GT {
			struct IncludeMacro *i = (struct IncludeMacro*)malloc(sizeof(struct IncludeMacro));
			i->headerFile = $3;
			$$ = i;
		}
		| INCLUDE DOUBLE_QUO HEADERID DOUBLE_QUO {
			struct IncludeMacro *i = (struct IncludeMacro*)malloc(sizeof(struct IncludeMacro));
			i->headerFile = $3;
			$$ = i;
		}
...
MAINFUNC: INT MAIN '(' VOID ')' COMPOUNDSTMT {
...
int main(int argc, char** argv) {
    ...
    startScope = createNewScope(GLOBAL_scope, NULL);
