...
int row_num;
Type_e currentType;		// used for distinguish the int and float type
...
void visitProgram(struct Program* program) {
	if (program->methodDef != NULL) {
		visitMethodDef(program->methodDef);
	}
...
void visitVarDecl(struct VarDecl* decl) {
	...
	if (decl->assignType != eNon) {
		...
		if (decl->assignType == eAsChar) fprintf(fp1, "%c;\n", decl->assigner.charnum);		
	} else {
		fprintf(fp1, ";\n");		
	}
}
...
void visitExpr(struct Expr* expr) {
	switch(expr->e) {
		...
		case eCharnum:
			fprintf(fp1, "%c", expr->type.charnum);
			break;
